...
function incrementalMedian(array $values)
{
    ...
    for ($x = 1; $x <= sizeof($values); $x++) {
        ...
        if (sizeof($selectedValues) % 2 == 0) {
            array_push($outputValues, ((float)$selectedValues[sizeof($selectedValues) / 2 - 1]));
        } else {
            array_push($outputValues, (float)$selectedValues[sizeof($selectedValues) / 2]);
        }
    }
...
var_dump(incrementalMedian($medianExample1));